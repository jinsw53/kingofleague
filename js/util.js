 ...
 * 🌟 sfx.diceRoll 추가: 게시판 주사위 굴림 애니메이션(0.85초, 78%/100% 지점 바운스·착지)과 타이밍을 맞춘 "데구르르" 합성 효과음
 */
Boako.Util = {
    // 💬 1. 알림창 띄우기 (기존 코드 그대로)
    toast: (msg) => {
...
window.sfx = (function() {
    ...
    return {
        ...
        // 🌟 [신규] 게시판 "오늘의 주사위" 전용 — 데구르르 굴러가는 소리 + 바운스 2번 + 착지 + 포인트 획득 차임
        // showDiceRollOverlay의 CSS 애니메이션(0.85초 완주, 78%=0.663초 지점 1차 바운스, 100%=0.85초 지점 최종 착지)과
        // 정확히 같은 숫자를 써서 싱크를 맞춤. 애니메이션 타이밍을 바꾸면 여기 숫자도 같이 바꿔야 함.
        diceRoll: function() {
            const rollEnd = 0.53; // 애니메이션 0~62% 구간(계속 회전하는 부분)에 대응
            let t = 0;
            while (t < rollEnd) {
                noiseBurst(0.035, 0.05, t);
                tone(180 + Math.random() * 180, 0.025, 'square', 0.02, t);
                t += 0.04 + Math.random() * 0.025; // 틱 간격을 살짝 랜덤하게 줘서 기계적으로 안 들리게
            }
            // 1차 바운스 (애니메이션 78% 지점 ≈ 0.663초)
            noiseBurst(0.05, 0.09, 0.663);
            tone(240, 0.06, 'triangle', 0.1, 0.663);
            // 최종 착지 (애니메이션 100% 지점 = 0.85초)
            noiseBurst(0.09, 0.16, 0.85);
            tone(130, 0.14, 'triangle', 0.18, 0.85);
            // 착지 직후 포인트 획득 차임
            tone(523.25, 0.1, 'sine', 0.13, 0.95);
            tone(783.99, 0.2, 'sine', 0.11, 1.03);
        },
       error: function() {
