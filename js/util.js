...
window.sfx = (function() {
    ...
    const raw = {
        ...
        // 🌟 [수정] 업적 달성 전용 — 저음 임팩트로 무게감을 주고, 상승 아르페지오를 옥타브로 두껍게 겹치고,
        // 마지막 화음을 C-E-G 3화음으로 길게 서스테인시켜서 훨씬 웅장하게 들리도록 재설계
        achievementUnlock: function() {
            // 1) 묵직한 저음 임팩트 (시작을 알리는 웅장한 타격감)
            tone(65.41, 0.5, 'triangle', 0.32, 0);     // C2 서브베이스
            tone(130.81, 0.4, 'sawtooth', 0.2, 0);     // C3 두께 보강
            noiseBurst(0.15, 0.24, 0);                  // 타격 질감

            // 2) 상승 아르페지오 (옥타브 더블링으로 풍성하게)
            tone(523.25, 0.16, 'triangle', 0.24, 0.12);  // C5
            tone(261.63, 0.16, 'triangle', 0.14, 0.12);  // C4 보강
            tone(659.25, 0.16, 'triangle', 0.24, 0.22);  // E5
            tone(329.63, 0.16, 'triangle', 0.14, 0.22);  // E4 보강
            tone(783.99, 0.16, 'triangle', 0.24, 0.32);  // G5
            tone(392.00, 0.16, 'triangle', 0.14, 0.32);  // G4 보강

            // 3) 메인 타격 화음 (C 메이저 3화음, 길게 서스테인)
            tone(1046.5, 0.65, 'triangle', 0.3, 0.44);   // C6
            tone(1318.5, 0.65, 'triangle', 0.24, 0.44);  // E6
            tone(1568.0, 0.65, 'triangle', 0.24, 0.44);  // G6
            tone(523.25, 0.65, 'sine', 0.16, 0.44);      // C5 sine (저음 보강)

            // 4) 반짝이는 배음 (하이엔드 스파클)
            tone(2093.0, 0.45, 'sine', 0.11, 0.48);      // C7
            tone(3136.0, 0.35, 'sine', 0.06, 0.52);      // G7
        },
