 ...
 * 🌟 랭킹 배너에 팀 리그 타이틀 스폰서(네이밍권) 배지 연결 (Boako.Util.applyTitleSponsorPrefix)
...
<button id="btn-tab-record" class="team-tab-btn bg-slate-100 text-slate-500 hover:bg-slate-200 hover:text-slate-700 px-4 py-2 rounded-lg text-sm font-bold transition-all" onclick="Boako.View.switchTeamTab('record')">⚔️ 대항전 기록 및 일정</button>
                                        ...
                                        ${members?.map(m => {
                                            ...
                                            <div class="member-item" ${isDraggable ? `draggable="true" ondragstart="Boako.Team.onMemberDragStart(event, '${m.player_name.replace(/'/g, "\\'")}')" ondragend="Boako.Team.onMemberDragEnd(event)" style="cursor:grab;"` : ''}>
                                                <div style="display:flex; align-items:center; gap:18px;">
                                                    ...
                                                    ${isDraggable ? `<small style="color:#94a3b8; font-size:11px;">↕ 드래그해서 포인트 지급</small>` : ''}
                                                </div>
                                                <div>
            ...
            case 'board':
                if (!Boako.Board || !Boako.Board.init) {
                    await Boako.Util.loadScript('js/board.js');
                }
                html = `<div id="board-master-container" class="w-full"></div>`;

                setTimeout(() => {
                    if (Boako.Board && typeof Boako.Board.init === 'function') {
                        Boako.Board.init('board-master-container');
                    }
                }, 0);
                break;
