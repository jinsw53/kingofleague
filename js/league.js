...
if (Boako.League.KOL && Boako.League.KOL.unsubscribeRealtime) {
    ...
    if (tabId === 'bingo') {
        ...
        container.innerHTML = `<div class="text-center py-20 font-black text-slate-400 animate-pulse">킹 오브 리그 데이터 동기화 중...</div>`;
        await Boako.League.loadKingOfLeagueData();
    }
...
Boako.League.initChallengeData = async function() {
    try {
        ...
        // 🚨 [리팩토링] 탭마다 소켓이 늘어나던 문제 방지 — realtime_coordinator.js 탭 리더 선출 패턴 적용
        // (achievements.js/rival_notify.js/auth.js와 동일 패턴). 리더 탭만 Supabase 채널을 구독하고,
        // 이벤트를 모든 탭에 relay해서 각 탭의 UI는 그대로 갱신됨.
        Boako.League._onChallengeChange = async (payload) => {
        ...
        Boako.League._onProfileChange = (payload) => {
        ...
        if (!Boako.League._coordinatorInited) {
            Boako.League._coordinatorInited = true;
            Boako.RealtimeCoordinator.onRelay('league:challenge-change', (p) => Boako.League._onChallengeChange(p));
            Boako.RealtimeCoordinator.onRelay('league:profile-change', (p) => Boako.League._onProfileChange(p));
            Boako.RealtimeCoordinator.onBecomeLeader(() => Boako.League._subscribeAsLeader());
        }
        Boako.League._subscribeAsLeader();
...
// 🚨 [리팩토링] 리더 탭에서만 실제 Supabase 채널을 구독 (탭 수만큼 소켓이 늘어나던 문제 방지)
Boako.League._subscribeAsLeader = function() {
    if (!Boako.RealtimeCoordinator.isLeader() || Boako.League.State.realtimeChannel) return;
    Boako.League.State.realtimeChannel = Boako.db
        ...
        .on('postgres_changes', { event: '*', schema: 'public', table: 'challenges' }, (payload) => {
            Boako.League._onChallengeChange(payload);
            Boako.RealtimeCoordinator.broadcast('league:challenge-change', payload);
        })
        .on('postgres_changes', { event: '*', schema: 'public', table: 'profiles' }, (payload) => {
            Boako.League._onProfileChange(payload);
            Boako.RealtimeCoordinator.broadcast('league:profile-change', payload);
        })
        .subscribe();
};
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-7 gap-6 items-start relative z-10">
                        <div class="md:col-span-3 space-y-4 relative transition-all duration-500" style="${atkStyle}">
                            ...
                            <div class="bg-white/90 backdrop-blur-sm border border-slate-200 p-4 rounded-2xl flex flex-col items-center text-center shadow-sm">
                                <img src="${Boako.Util.cdn(p.attacker_team_logo_url || 'https://qrredwrxdnvqwdxzanba.supabase.co/storage/v1/object/public/teams/etc/default_logo.png')}" class="w-16 h-16 rounded-xl object-contain bg-slate-50 border border-slate-100 p-1 mb-2 shadow-inner">
                        ...
                        <div class="md:col-span-3 space-y-4 relative transition-all duration-500" style="${defStyle}">
                            ${defStamp}
                            <div class="bg-white/90 backdrop-blur-sm border border-slate-200 p-4 rounded-2xl flex flex-col items-center text-center shadow-sm">
                                <img src="${Boako.Util.cdn(p.defender_team_logo_url || 'https://qrredwrxdnvqwdxzanba.supabase.co/storage/v1/object/public/teams/etc/default_logo.png')}" class="w-16 h-16 rounded-xl object-contain bg-slate-50 border border-slate-100 p-1 mb-2 shadow-inner">
...
const isPendingState = currentStatus === 'PENDING' || prevStatus === 'PENDING';
        ...
        card.className = `p-5 rounded-3xl border ${currentStatus.startsWith('CANCELED') ? 'bg-slate-100/50 border-slate-200 opacity-50' : currentStatus === 'NEGOTIATING' ? 'bg-amber-50/30 border-amber-300' : 'bg-white border-slate-200'} transition-all shadow-sm flex flex-col md:flex-row gap-5 md:items-center relative group`;
            ...
            <div class="flex-1 flex flex-col min-w-0 gap-3">
                <div class="flex items-center gap-2">
                    ...
                    <div class="flex items-center gap-1">
                        ${p.is_attacker_token_used ? `<span class="bg-amber-50 text-amber-600 border border-amber-200 text-[9px] font-black px-1.5 py-0.5 rounded">도전팀 <img src="${Boako.Util.cdn('https://qrredwrxdnvqwdxzanba.supabase.co/storage/v1/object/public/teams/etc/challengetoken.png')}" class="w-3 h-3 inline-block"></span>` : ''}
                        ${p.is_defender_token_used ? `<span class="bg-orange-50 text-orange-600 border border-orange-200 text-[9px] font-black px-1.5 py-0.5 rounded">응전팀 <img src="${Boako.Util.cdn('https://qrredwrxdnvqwdxzanba.supabase.co/storage/v1/object/public/teams/etc/challengetoken.png')}" class="w-3 h-3 inline-block"></span>` : ''}
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="flex flex-wrap gap-1.5">
                        ${isPendingState 
                            ? gamesHtml 
                            : `<div class="flex items-center gap-2 bg-slate-800 text-white px-3 py-1.5 rounded-xl shadow-sm"><img src="${Boako.Util.cdn((p.game_logo_url && p.game_logo_url !== 'null') ? p.game_logo_url : 'https://qrredwrxdnvqwdxzanba.supabase.co/storage/v1/object/public/teams/etc/challenge%20(1).png')}" class="w-4 h-4 object-contain rounded bg-white p-0.5" /><span class="text-xs font-black">${p.game_name}</span><span class="text-[9px] font-black text-slate-400 border-l border-slate-600 pl-2 ml-1">${Boako.League.formatMode(p.game_mode)}</span></div>`
                        }
                    </div>
                </div>
                    ...
                    <div class="bg-indigo-50 p-3 border-b border-indigo-100 flex items-center gap-2">
                        ...
                        <div class="flex-1">
                            ${Boako.Util.renderCSelect(
                                ...
                                'Boako.League.Calendar.changeFixedTime'
                            )}
                        </div>
                    </div>
...
Boako.League.showAcceptPopup = function(challengeId) {
    ...
    const safeTeamLogo = Boako.Util.cdn((p.attacker_team_logo_url && p.attacker_team_logo_url !== 'null') ? p.attacker_team_logo_url : 'https://qrredwrxdnvqwdxzanba.supabase.co/storage/v1/object/public/teams/etc/default_logo.png');
    ...
    scoreData.forEach(row => {
        ...
        html += `
            <div class="p-4 bg-white border border-slate-200/80 rounded-2xl shadow-sm space-y-3 transition-all hover:border-slate-300">
                <div class="flex justify-between items-center"><span class="font-black text-slate-800 text-xs flex items-center gap-1.5"><img src="${Boako.Util.cdn(teamLogoUrl)}" class="w-4 h-4 object-contain rounded-full shadow-sm bg-slate-50 border border-slate-100"> ${teamName}</span><div class="text-right flex items-center"><span class="text-indigo-600 font-black text-sm ml-0.5"><span class="flex flex-col items-end leading-tight">🏆 ${totalScore} LP<span class="text-slate-400 font-bold">획득</span></span></span></div></div>
...
Boako.League.showChallengeRuleModal = async function() {
    ...
    const TOKEN_IMG = `<img src="${Boako.Util.cdn('https://qrredwrxdnvqwdxzanba.supabase.co/storage/v1/object/public/teams/etc/challengetoken.png')}" class="w-4 h-4 inline-block align-middle mx-0.5">`;
    const modalHtml = `
...
Boako.League.drawChampionRows = function(dataList) {
    ...
    <tr class="hover:bg-slate-50/50 transition-colors">
       ...
       <td class="p-4"><div class="flex items-center gap-2"><div class="w-8 h-8 rounded-full overflow-hidden flex items-center justify-center shrink-0"><img src="${Boako.Util.cdn('https://qrredwrxdnvqwdxzanba.supabase.co/storage/v1/object/public/teams/etc/CHAMPION.png')}" alt="CHAMPION BADGE" class="w-full h-full object-contain"></div><span class="font-extrabold text-slate-900">${mvpName}</span></div></td>
       <td class="p-4 text-slate-500 font-bold relative group/handler">
            <div class="flex items-center gap-2">
                <div class="w-6 h-6 rounded-full border border-slate-200 bg-slate-50 flex items-center justify-center shadow-sm shrink-0 relative cursor-pointer"><img src="${Boako.Util.cdn(row.mvp_team_logo) || 'https://qrredwrxdnvqwdxzanba.supabase.co/storage/v1/object/public/teams/etc/challenge%20(1).png'}" alt="TEAM LOGO" class="w-full h-full object-contain rounded-full"></div>
                ...
                <div class="invisible opacity-0 group-hover/handler:visible group-hover/handler:opacity-100 fixed -translate-x-1/2 -translate-y-full mb-2 w-32 h-32 p-2 bg-white border border-slate-200 rounded-2xl shadow-xl z-[9999] transition-all duration-200 pointer-events-none flex items-center justify-center" style="top: var(--tooltip-top, auto); left: var(--tooltip-left, auto);">
                    <img src="${Boako.Util.cdn(row.mvp_team_logo) || 'https://qrredwrxdnvqwdxzanba.supabase.co/storage/v1/object/public/teams/etc/challenge%20(1).png'}" alt="LARGE TEAM LOGO" class="w-full h-full object-contain">
...
Boako.League.loadKingOfLeagueData = async function() {
    ...
    try {
        ...
        const activeTeamIds = [...new Set((progressRows || []).map(r => r.team_id))];

        let teamsInfo = [];
        if (activeTeamIds.length > 0) {
            const { data: teamsData } = await Boako.db
                .from('teams')
        ...
        (progressRows || []).forEach(r => {
            if (r.is_completed) {
                completedCountMap[r.team_id] = (completedCountMap[r.team_id] || 0) + 1;
            }
        });
        ...
        (eventsData || []).forEach(e => {
            ...
            if (e.defender_team_id) eventTeamIds.add(e.defender_team_id);
        });
        const missingIds = [...eventTeamIds].filter(id => !teamsInfo.find(t => t.id === id));
        let extraTeams = [];
        if (missingIds.length > 0) {
            const { data: extraData } = await Boako.db.from('teams').select('id, team_name, logo_url').in('id', missingIds);
            extraTeams = extraData || [];
        }
        const allTeamsMap = {};
        ...
        const { data: settlementsData } = await Boako.db
            ...
            .select('*')
            .eq('season_no', seasonNo)
            .order('settlement_round', { ascending: true })
            .order('lp_awarded', { ascending: false });

        Boako.League.KOL.teams = teamsCombined;
        ...
        container.innerHTML = `
            <div class="flex flex-col items-center justify-center py-20 text-center bg-white border border-slate-200 rounded-2xl shadow-sm">
                ...
                <h3 class="font-black text-slate-800 text-lg mb-2">데이터 로드 오류</h3>
                <button onclick="Boako.League.loadKingOfLeagueData()" class="mt-6 bg-slate-800 text-white font-black text-xs px-6 py-3 rounded-xl hover:bg-slate-900 transition-all shadow-md">재시도</button>
            </div>
        `;
        if (window.lucide) window.lucide.createIcons();
    }
...
Boako.League.getKingOfLeagueHTML = function() {
    ...
    const arenaCenterContent = seasonLogoUrl
        ? `<img src="${Boako.Util.cdn(seasonLogoUrl)}" class="kol-arena-center-logo" alt="시즌 로고">`
    ...
    const settlementsByRound = { 1: [], 2: [], 3: [] };
    (state.settlements || []).forEach(s => {
        if (settlementsByRound[s.settlement_round]) settlementsByRound[s.settlement_round].push(s);
    });

    const settlementHtml = [1, 2, 3].map(round => {
        const rows = settlementsByRound[round];
        if (rows.length === 0) {
            return `
                <div class="p-4 bg-slate-50 border border-dashed border-slate-200 rounded-xl">
                    <div class="text-[10px] font-black text-slate-400 uppercase tracking-wider mb-1">${roundLabels[round]}</div>
                    <div class="text-xs font-bold text-slate-400">아직 정산되지 않았습니다.</div>
                </div>
            `;
        }
        const totalPool = rows[0].total_lp_pool;
        const rowsHtml = rows.map((r, idx) => `
            <div class="flex items-center justify-between py-1.5 ${idx !== rows.length - 1 ? 'border-b border-slate-100' : ''}">
                <span class="text-xs font-black text-slate-700">${idx + 1}. ${r.team_name}</span>
                <span class="text-xs font-black text-indigo-600">🏆 ${r.lp_awarded.toLocaleString()} LP <span class="text-slate-400 font-bold">(하트 ${r.hearts_at_settlement})</span></span>
            </div>
        `).join('');
        return `
    ...
    return `
        <div class="space-y-8">
            ...
            <div class="space-y-3">
                <h5 class="font-black text-slate-800 text-sm flex items-center gap-2"><i data-lucide="coins" class="w-4 h-4 text-amber-500"></i> LP 정산 현황</h5>
...
// 🌟 [리팩토링] 이 화면(킹오브리그 빙고 탭)을 여러 탭에서 열어두면 탭마다 각자 채널을 구독해서
// 소켓이 늘어나던 문제 방지 — realtime_coordinator.js의 화면 전용 미니 코디네이터(createGroup) 적용.
Boako.League.KOL.subscribeRealtime = function() {
    if (!Boako.League.KOL._rtGroup) {
        Boako.League.KOL._rtGroup = Boako.RealtimeCoordinator.createGroup('kol-events');
        Boako.League.KOL._rtGroup.onRelay('event', (ev) => Boako.League.KOL._handleEvent(ev));
        Boako.League.KOL._rtGroup.onBecomeLeader(() => Boako.League.KOL._subscribeAsLeader());
    }
    Boako.League.KOL._rtGroup.start();
};

// 🌟 이 탭이 리더일 때만(그리고 아직 구독 안 했을 때만) 실제 채널 구독
Boako.League.KOL._subscribeAsLeader = function() {
    ...
    Boako.League.KOL.realtimeChannel = Boako.db
        ...
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'kol_events' }, (payload) => {
            Boako.League.KOL._handleEvent(payload.new);
            Boako.League.KOL._rtGroup.broadcast('event', payload.new);
        })
        .subscribe();
};

// 🌟 리더/팔로워 공통 반응 로직
Boako.League.KOL._handleEvent = function(ev) {
...
Boako.League.KOL.unsubscribeRealtime = function() {
    ...
    if (Boako.League.KOL._rtGroup) {
        Boako.League.KOL._rtGroup.teardown();
        Boako.League.KOL._rtGroup = null;
    }
};
