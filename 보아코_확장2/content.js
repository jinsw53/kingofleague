  ...
  mutations.forEach((mutation) => {
    ...
    if (mutation.type === "attributes") {
      ...
      if (target.id === "game_result_panel") {
        ...
        target.id === "overall-content" &&
        target.classList.contains("gamestate_gameEnd") &&
        target.getAttribute("data-boako-live-result-handled") !== "true"
      ) {
        // 라이브 게임 페이지: game_result_panel이 사라지고 pagesection_gameresult로
        // 교체되는 시점을 잡기 위한 별도 트리거 (기존 game_result_panel 로직과 무관)
        target.setAttribute("data-boako-live-result-handled", "true");
        console.log("[확인] 라이브 게임 종료 감지 (gamestate_gameEnd) - 자동저장 버튼 추가 시도");
        addLiveSaveButton();
      }
    }
  });
...
// ========================================================================
// 라이브 게임 페이지( /숫자/게임명?table=숫자 ) 전용 자동저장 로직
// 게임 종료 시 #game_result_panel이 사라지고 #pagesection_gameresult로
// 교체되므로, 기존 addSaveButtons()/extractGameData()와는 완전히 별도로 동작한다.
...
function addLiveSaveButton(retryCount = 0) {
  ...
  // 중복 방지 (기존 addSaveButtons()와 클래스를 공유하므로 어느 쪽이 먼저 붙여도 하나만 생성됨)
  if (document.querySelector('.boako-save-button')) {
    return;
  }
  ...
  console.log("[실행] addLiveSaveButton 함수 실행 시작");
  ...
  resultTitle.after(button);
  console.log("[확인] (라이브) '확인중...' 버튼 DOM에 추가됨");

  setTimeout(() => {
    const userNickname = getCurrentUserNickname();
    if (!userNickname) {
      console.log("로그인 되지 않음 (라이브)");
      let existingButton = document.querySelector(".bga-save-button");
      if (existingButton) {
        existingButton.remove();
      }
    ...
    // 초기 버튼 상태 업데이트
    updateLiveButtonState(button);

    // 라이브 결과 영역 내용 변경 감지 (버튼 상태 갱신용, 디바운스 적용)
    let liveContentUpdateTimer = null;
    const liveResultContentObserver = new MutationObserver((mutations) => {
      let shouldUpdate = false;
      mutations.forEach((mutation) => {
        if (mutation.type === "childList" || mutation.type === "characterData") {
          shouldUpdate = true;
      ...
      if (shouldUpdate) {
        if (liveContentUpdateTimer) {
          clearTimeout(liveContentUpdateTimer);
        }
        liveContentUpdateTimer = setTimeout(() => {
          updateLiveButtonState(button);
          liveContentUpdateTimer = null;
        }, 80);
      }
    });

    const pagesectionGameresult = document.querySelector('#pagesection_gameresult');
    if (pagesectionGameresult) {
      liveResultContentObserver.observe(pagesectionGameresult, {
        childList: true,
        subtree: true,
        characterData: true,
      });
    ...
    button.onclick = () => {
      if (button.disabled) {
        console.log("[정보] (라이브) 기록이 불가능한 게임이므로 클릭이 차단됨.");
        showPopupMessage("이 게임은 기록할 수 없습니다.", false, button);
        return;
      }
      ...
      function tryExtractData(attempt = 1, maxAttempts = 3) {
        const gameData = extractLiveGameData();
        ...
        if (gameData === null) {
          console.log("[오류] (라이브) 게임 결과 데이터를 추출하지 못해 저장을 중단합니다.");
          resetButton(button, originalText);
          showPopupMessage("게임 결과를 아직 읽지 못했습니다. 잠시 후 다시 시도해주세요.", false, button);
        } else {
          sendToBackground(gameData, button, originalText);
    ...
    console.log("[확인] (라이브) '기록 저장' 버튼 추가됨");

    // 버튼 상태가 활성화되어 있으면 자동 클릭
    setTimeout(() => {
      if (button.getAttribute('data-autoclick-attempted') === 'true') {
        return;
      }
      button.setAttribute('data-autoclick-attempted', 'true');

      if (!button.disabled) {
        console.log("[확인] (라이브) 버튼 자동 클릭 실행");
        button.click();
      } else {
        console.log("[정보] (라이브) 버튼이 비활성화 상태여서 자동 클릭 실행되지 않음");
      }
    }, 500);
  }, 800);
}

// 라이브 페이지용 버튼 상태 업데이트 (updateExistingButtonState와 동일한 판단 로직이되
// #game_result_panel 대신 extractLiveGameData()로 참여/승리 여부를 판단)
function updateLiveButtonState(button) {
  let isCancelled = [
    "#game_abandonned",
    "#game_cancelled",
    "#game_unranked_cancelled",
...
function extractLiveGameData() {
  ...
  scoreEntries.forEach((entry) => {
    const rank = entry.querySelector(".rank")?.innerText.trim();
    const nickname = entry.querySelector("a.playername, .playername")?.innerText.trim();
    const scoreElement = entry.querySelector(".score");
    let score = "";
    if (scoreElement) {
      const scoreText = scoreElement.innerText.trim();
  ...
  // 전역 데이터를 우선 사용하고, 없으면 쿼리스트링에서 보조 추출
  let gameId = "unknown";
  if (typeof bgaGameData !== "undefined" && bgaGameData && bgaGameData.tableId) {
    gameId = String(bgaGameData.tableId);
  } else {
  ...
  if (gameType === "협력") {
    const winners = players
      .filter((p) => p.rank && p.rank.includes("승자"))
      .map((p) => p.nickname)
      .filter((n) => n != null && n !== "");
    winner = winners.length > 0 ? winners.join(",") : "";
  } else {
    let winners = players
      .filter((p) => p.rank && (p.rank.includes("1위") || p.rank.includes("승자")))
      .map((p) => p.nickname)
      .filter((n) => n != null && n !== "");
    if (winners.length === 0 && players.length > 0 && players[0].nickname) {
      winners = [players[0].nickname];
    }
    winner = winners.length > 0 ? winners.join(",") : "No Winner";
  }

  if (!winner || winner === "" || winner === "No Winner") return null;
