/**
 * 🎯 [LEAGUE] 실시간 리그 콘텐츠 전당 (소장님 오리지널 기획본)
 * 관리 책임자: 소장님 MASTER
...
Boako.League.State = {
    ...
    leaderboard: [
        { rank: 1, name: '소장님', team: '소장님 패밀리', game: '스플렌더', tier: 'Grandmaster 👑', rating: 3105 },
        { rank: 2, name: 'ArchLord', team: 'Team Arch', game: '아크 노바', tier: 'Master', rating: 2890 },
        { rank: 3, name: 'GoldStar', team: 'Team Gold', game: '윙스팬', tier: 'Master', rating: 2750 },
        { rank: 4, name: 'Odin001', team: 'Team Odin', game: '쿼리도', tier: 'Diamond 1', rating: 2610 },
        { rank: 5, name: 'GamerX', team: '레드 피닉스', game: '센추리', tier: 'Diamond 2', rating: 2430 }
    ],
    boardGames25: [
        ...
        <div class="w-full max-w-4xl mx-auto bg-white border border-slate-200/80 rounded-3xl premium-shadow overflow-hidden transition-all duration-300">
            ...
            <div class="p-6 sm:p-8 border-b border-slate-100 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 bg-slate-50/50">
                ...
                <div class="grid grid-cols-2 sm:flex items-center gap-1.5 p-1 bg-slate-100/90 rounded-2xl border border-slate-200/50 w-full md:w-auto">
                    ...
                    <button id="tab-ranking" onclick="Boako.League.switchTab('ranking')" class="flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-xl text-xs font-black transition-all duration-200">
                        <span>👑 킹 오브 리그</span>
                    </button>
...
// 💡 3. 탭 스위칭 컨트롤러
Boako.League.switchTab = function(tabId) {
    if (typeof sfx !== 'undefined') sfx.playClick();
    Boako.League.State.currentTab = tabId;

    const tabs = ['bingo', 'provocation', 'ranking', 'tournament'];
    tabs.forEach(t => {
    ...
    if (tabId === 'bingo') {
        ...
        container.innerHTML = Boako.League.getRankingHTML();
        Boako.League.renderLeaderboard();
...
Boako.League.selectTeam = function(teamId) {
    ...
    teams.forEach(t => {
        ...
        if (btn) {
            btn.className = (t === teamId) 
                ? `px-4 py-2 text-xs font-bold rounded-xl transition-all ${classConfig[t]}`
                : "px-4 py-2 text-xs font-bold rounded-xl transition-all text-slate-500 hover:bg-slate-100";
        }
    });
    if (typeof showToast !== 'undefined') showToast(`${teamId.toUpperCase()} 가문으로 영토 점거 준비 완료!`);
};
...
Boako.League.renderBingoBoard = function() {
    ...
    const teamStyles = {
        blue: 'bg-blue-600 text-white border-blue-400 shadow-inner scale-[0.97]',
        red: 'bg-red-600 text-white border-red-400 shadow-inner scale-[0.97]',
        emerald: 'bg-emerald-600 text-white border-emerald-400 shadow-inner scale-[0.97]',
        amber: 'bg-amber-500 text-slate-950 border-amber-300 font-extrabold shadow-inner scale-[0.97]'
    };
    ...
    Boako.League.State.bingoBoard.forEach((owner, idx) => {
        ...
        cell.className = `h-20 rounded-2xl border flex flex-col items-center justify-center p-1.5 transition-all text-center relative overflow-hidden group ${
            owner 
            ? `${teamStyles[owner]} ${isWinner ? 'bingo-won-pulse border-2' : ''}`
            : 'bg-slate-50 border-slate-200/70 hover:border-violet-500 hover:bg-slate-100/50'
        }`;
        ...
        cell.innerHTML = `
            <span class="text-[9px] font-black tracking-wider text-slate-400 group-hover:text-violet-600 transition-colors uppercase">TILE ${idx+1} ${flag}</span>
            <span class="text-xs font-black truncate w-full tracking-tight mt-1">${Boako.League.State.boardGames25[idx]}</span>
            ${isWinner ? '<span class="absolute bottom-1 right-1.5 text-xs text-amber-400">★</span>' : ''}
        `;
...
Boako.League.toggleTile = function(index) {
    if (typeof sfx !== 'undefined') sfx.playClick();
    if (Boako.League.State.bingoBoard[index] === Boako.League.State.selectedTeam) {
        Boako.League.State.bingoBoard[index] = null;
    } else {
        Boako.League.State.bingoBoard[index] = Boako.League.State.selectedTeam;
    }
    Boako.League.renderBingoBoard();
};
...
Boako.League.calculateWinningCells = function() {
    ...
    const board = Boako.League.State.bingoBoard;

    for (let r = 0; r < size; r++) {
        let base = board[r * size];
        if (base) {
            let match = true;
            for (let c = 1; c < size; c++) if (board[r * size + c] !== base) match = false;
            if (match) for (let c = 0; c < size; c++) winningSet.add(r * size + c);
        }
    }
    for (let c = 0; c < size; c++) {
        ...
        if (base) {
            let match = true;
            for (let r = 1; r < size; r++) if (board[r * size + c] !== base) match = false;
            if (match) for (let r = 0; r < size; r++) winningSet.add(r * size + c);
        }
    }
    let baseDiag1 = board[0];
    if (baseDiag1) {
        let match = true;
        for (let i = 1; i < size; i++) if (board[i * size + i] !== baseDiag1) match = false;
        if (match) for (let i = 0; i < size; i++) winningSet.add(i * size + i);
    }
    let baseDiag2 = board[size - 1];
    if (baseDiag2) {
        let match = true;
        for (let i = 1; i < size; i++) if (board[i * size + (size - 1 - i)] !== baseDiag2) match = false;
        if (match) for (let i = 0; i < size; i++) winningSet.add(i * size + (size - 1 - i));
    }
...
Boako.League.countLinesForTeam = function(team) {
    ...
    if (Array(size).fill(0).every((_, i) => board[i * size + i] === team)) lines++;
    if (Array(size).fill(0).every((_, i) => board[i * size + (size - 1 - i)] === team)) lines++;
    return lines;
};

Boako.League.updateStats = function() {
    const counts = { blue: 0, red: 0, emerald: 0, amber: 0 };
    Boako.League.State.bingoBoard.forEach(owner => { if(owner) counts[owner]++; });

    const setVal = (id, val) => {
        const el = document.getElementById(id);
        if (el) el.innerText = val;
    };
    setVal('stat-blue-tiles', counts.blue);
    setVal('stat-blue-lines', Boako.League.countLinesForTeam('blue'));
    setVal('stat-red-tiles', counts.red);
    setVal('stat-red-lines', Boako.League.countLinesForTeam('red'));
    setVal('stat-emerald-tiles', counts.emerald);
    setVal('stat-emerald-lines', Boako.League.countLinesForTeam('emerald'));
    setVal('stat-amber-tiles', counts.amber);
    setVal('stat-amber-lines', Boako.League.countLinesForTeam('amber'));
};

Boako.League.randomizeBoard = function() {
    if (typeof sfx !== 'undefined') sfx.playBingo();
    const options = [null, 'blue', 'red', 'emerald', 'amber', null];
    Boako.League.State.bingoBoard = Boako.League.State.bingoBoard.map(() => options[Math.floor(Math.random() * options.length)]);
    Boako.League.renderBingoBoard();
    if (typeof showToast !== 'undefined') showToast("영토가 불규칙하게 무작위 배치되었습니다!");
};

Boako.League.clearBoard = function() {
    if (typeof sfx !== 'undefined') sfx.playClick();
    Boako.League.State.bingoBoard = Array(25).fill(null);
    Boako.League.renderBingoBoard();
    if (typeof showToast !== 'undefined') showToast("모든 요새 점유 상황이 복구되었습니다.");
};
    ...
    return `
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="bg-slate-50 border border-slate-200/80 rounded-2xl p-5 space-y-4 h-fit">
                ...
                <div class="space-y-3 text-xs font-bold text-slate-600">
                    <div>
                        ...
                        <select id="prov-attacker" class="w-full bg-white border border-slate-200 rounded-xl p-2.5 outline-none focus:border-violet-500">
                    ...
                    <div>
                        ...
                        <select id="prov-defender" class="w-full bg-white border border-slate-200 rounded-xl p-2.5 outline-none focus:border-violet-500">
                            <option value="레드 피닉스">❤️ 레드 피닉스</option>
                            <option value="블루 타이거">💙 블루 타이거</option>
                            <option value="그린 드래곤">💚 그린 드래곤</option>
                            <option value="골드 세이버">💛 골드 세이버</option>
                    ...
                    <div>
                        ...
                        <select id="prov-game" class="w-full bg-white border border-slate-200 rounded-xl p-2.5 outline-none focus:border-violet-500">
                    ...
                    <div>
                        ...
                        <textarea id="prov-msg" rows="3" class="w-full bg-white border border-slate-200 rounded-xl p-2.5 outline-none focus:border-violet-500" placeholder="상대의 사기를 무너뜨릴 한마디를 적어보세요."></textarea>
                    </div>
                </div>
                <button onclick="Boako.League.registerProvocation()" class="w-full bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-700 hover:to-indigo-700 text-white font-black text-xs py-3 rounded-xl shadow-lg transition-all">🔥 도발장 릴리즈</button>
            </div>
            ...
            <div class="lg:col-span-2 bg-slate-50 border border-slate-200/80 rounded-2xl p-5">
                ...
                <div id="provocation-list" class="space-y-4"></div>
            </div>
        </div>
    `;
};
...
Boako.League.renderProvocations = function() {
    const container = document.getElementById('provocation-list');
    if (!container) return;
    container.innerHTML = '';
    ...
    Boako.League.State.provocations.forEach(p => {
        ...
        card.innerHTML = `
            ...
            <p class="text-sm font-semibold text-slate-600 bg-slate-50/50 p-3 rounded-xl border border-slate-100 mb-3 italic">
                "${p.message}"
            </p>
            <div class="flex items-center justify-between">
                ...
                ${p.accepted 
                    ...
                    : `<button onclick="Boako.League.acceptProvocation(${p.id})" class="bg-violet-600 hover:bg-violet-700 text-white font-black text-[11px] px-3.5 py-1.5 rounded-lg shadow-sm">결투 대폭 수락</button>`
                }
            </div>
        `;
...
Boako.League.registerProvocation = function() {
    const attacker = document.getElementById('prov-attacker').value;
    const defender = document.getElementById('prov-defender').value;
    const game = document.getElementById('prov-game').value;
    const message = document.getElementById('prov-msg').value || "정정당당히 필드에서 무릎을 꿇려드리겠습니다.";

    if (attacker === defender) {
        if (typeof showToast !== 'undefined') showToast("동일한 팀을 향해 내분을 선포할 수 없습니다.");
        return;
    }

    if (typeof sfx !== 'undefined') sfx.playBingo();
    Boako.League.State.provocations.unshift({
        id: Boako.League.State.provocations.length + 1,
        attacker, defender, game, message, accepted: false
    });

    document.getElementById('prov-msg').value = '';
    Boako.League.renderProvocations();
    if (typeof showToast !== 'undefined') showToast("결투 도발장이 아레나에 정식 서명 유포되었습니다!");
};

Boako.League.acceptProvocation = function(id) {
    if (typeof sfx !== 'undefined') sfx.playBingo();
    const target = Boako.League.State.provocations.find(p => p.id === id);
    if (target) {
        target.accepted = true;
        Boako.League.renderProvocations();
        if (typeof showToast !== 'undefined') showToast(`${target.defender} 가문이 이 도발을 회피하지 않고 수락했습니다!`);
    }
};
...
// 👑 탭 3: 킹 오브 리그
// ==========================================
Boako.League.getRankingHTML = function() {
    return `
        <div class="space-y-4">
            <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                <h5 class="font-black text-slate-800 text-sm flex items-center gap-2">
                    <i data-lucide="trophy" class="w-4 h-4 text-amber-500"></i> 실시간 가문 랭킹 리드보드
                </h5>
                <div class="relative w-full max-w-xs">
                    <input onkeyup="Boako.League.renderLeaderboard()" id="ranking-search" type="text" class="w-full bg-slate-50 border border-slate-200 rounded-xl pl-3.5 pr-10 py-1.5 text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:border-violet-500 focus:bg-white" placeholder="플레이어 혹은 소속 팀 검색...">
            ...
            <div class="border border-slate-200/80 rounded-2xl overflow-hidden bg-white shadow-sm overflow-x-auto">
                <table class="w-full text-left text-xs min-w-[600px]">
                    <thead class="bg-slate-50 text-slate-500 font-bold border-b border-slate-200">
                        <tr>
                            <th class="p-4 text-center">순위</th>
                            <th class="p-4">대표 리거</th>
                            <th class="p-4">소속 가문</th>
                            <th class="p-4">주종목</th>
                            <th class="p-4">지위 계급</th>
                            <th class="p-4 text-right">보유 LP</th>
                        </tr>
                    </thead>
                    <tbody id="ranking-tbody" class="divide-y divide-slate-100 font-semibold text-slate-700"></tbody>
                </table>
            </div>
        </div>
    `;
...
Boako.League.renderLeaderboard = function() {
    ...
    Boako.League.State.leaderboard.forEach((player, i) => {
        if (query && !player.name.toLowerCase().includes(query) && !player.team.toLowerCase().includes(query)) return;
        ...
        tr.innerHTML = `
            <td class="p-4 text-center font-black text-amber-500">${medal}</td>
            <td class="p-4">
                <div class="flex items-center gap-2">
                    <div class="w-7 h-7 rounded-full bg-violet-50 border border-violet-100 flex items-center justify-center font-bold text-violet-600 text-xs">${player.name[0]}</div>
                    <span class="font-bold text-slate-800">${player.name}</span>
                </div>
            </td>
            <td class="p-4 text-slate-500 font-bold">${player.team}</td>
            <td class="p-4">
                <span class="bg-violet-50 border border-violet-100 text-violet-700 text-[10px] font-black px-2.5 py-0.5 rounded-md">${player.game}</span>
            </td>
            <td class="p-4 text-slate-400 font-semibold text-xs">${player.tier}</td>
            <td class="p-4 text-right font-black text-violet-600 text-sm">${player.rating.toLocaleString()} LP</td>
        `;
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 items-center max-w-3xl mx-auto pt-4">
                <div class="space-y-8">
                    <div class="bg-slate-50 border border-slate-200/80 p-4 rounded-2xl space-y-2">
                        ...
                        <div class="flex items-center justify-between p-1.5 bg-violet-50 text-violet-800 rounded-lg px-3 text-xs font-black border border-violet-100">
                            <span>💙 Team Arch</span>
                            <span>3</span>
                        </div>
                        <div class="flex items-center justify-between p-1.5 text-slate-400 px-3 text-xs font-bold">
                            <span>❤️ Team Odin</span>
                            <span>1</span>
                        </div>
                    </div>
                    <div class="bg-slate-50 border border-slate-200/80 p-4 rounded-2xl space-y-2">
                ...
                <div class="flex flex-col justify-around h-full py-8 text-center text-xs text-slate-400 font-bold hidden md:flex">
                    <div class="flex items-center justify-center gap-1.5 bg-slate-100/80 border border-slate-200 p-2 rounded-xl">
                        <span>Arch 승전</span> <i data-lucide="arrow-right" class="w-4 h-4 text-violet-500"></i>
                    </div>
                    <div class="flex items-center justify-center gap-1.5 bg-slate-100/80 border border-slate-200 p-2 rounded-xl">
                        <span>Gold 승전</span> <i data-lucide="arrow-right" class="w-4 h-4 text-violet-500"></i>
                    </div>
                </div>
                ...
                <div class="bg-gradient-to-br from-amber-500/5 to-orange-500/5 border border-amber-300 p-5 rounded-3xl text-center space-y-3.5 relative overflow-hidden">
                    <div class="text-[11px] font-black text-amber-700 tracking-wider flex items-center justify-center gap-1">
                        <span>🏆</span> GRAND CHAMPIONSHIP
                    </div>
                    <div class="bg-violet-600 text-white p-2.5 rounded-xl text-xs font-black flex justify-between shadow-sm">
                        <span>💙 Team Arch</span>
                        <span class="bg-violet-800 text-[10px] px-1.5 rounded">수성 세력</span>
                    </div>
                    <div class="p-0.5 text-slate-400 font-black text-xs">VS</div>
                    <div class="bg-amber-500 text-slate-950 p-2.5 rounded-xl text-xs font-black flex justify-between">
                        <span>💛 Team Gold</span>
                        <span class="bg-amber-600 text-[10px] text-slate-950 px-1.5 rounded">도전 세력</span>
                    </div>
                    <button onclick="if(typeof sfx !== 'undefined') sfx.playBingo(); if(typeof showToast !== 'undefined') showToast('정규 시즌 최종 챔피언 매치는 곧 오프라인에서 생중계됩니다!');" class="w-full bg-amber-500 hover:bg-amber-600 text-slate-950 font-black text-xs py-2.5 rounded-xl transition-all shadow-md mt-2">대진 상황 예측</button>
                </div>
            </div>
        </div>
    `;
